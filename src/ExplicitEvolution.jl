...
function symbolic_2_explicit_worker(angles, j_idx_arr, trigonometric_history_arr, angle_history_arr)
    ...
    for (i, j) in enumerate(j_idx_arr)
        ...
        if !isapprox(abs2(coeff), 0.0, atol=weight_cutoff)
            ...
            push!(j_idx_arr_contr, j) # only keep idxs of contributing coefficients, i.e. coeff ≠ 0
        end
    end
    return j_idx_arr_contr, coeff_arr
end
...
function explicit_final_state_projection_symbolic_backend(N, M, j_out, angles)
    ...
    j_idx_arr_contr, coeff_arr = sp_2_two_photon(N, j_idx_arr_l, j_idx_arr_m, coeff_arr_l, coeff_arr_m)
...
function explicit_final_state_projection_mesh(N, M, j_out, angles)
    ...
    l_out, c_out, m_out, k_out = j2lcmk(N+M, j_out)
    if c_out == 0
        l_init_max = min(N-1, l_out) # light cone for contributions fron the initial state
    else
        l_init_max = min(N-1, l_out - 1) # |n,S> cannot contribute to |n,L>, at most |n-1,S> can
    end
    if k_out == 0
        m_init_max = min(N-1, m_out) # light cone for contributions fron the initial state
    else
        m_init_max = min(N-1, m_out - 1) # |n,S> cannot contribute to |n,L>, at most |n-1,S> can
    end
