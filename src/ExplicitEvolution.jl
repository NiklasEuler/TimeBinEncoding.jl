...
export explicit_final_state_projection_sp, explicit_final_state_projection,
    explicit_final_state_coherence_map, explicit_add_final_state_projection
...
See also  `explicit_ket_evolution`, `explicit_final_state_projection`,
`explicit_final_state_projection_sp`
"""
function explicit_ket_evolution_sp(l, angles)
    ...
    j_idx_arr_contr, coeff_arr = symbolic_2_explicit_worker(angles, j_idx_arr,
        trigonometric_history_arr, angle_history_arr)
...
See also `explicit_final_state_projection`, `explicit_ket_evolution_sp`,
`explicit_final_state_coherence_map`
"""
function explicit_final_state_projection_sp(l, c, angles)
    ...
    j_idx_arr_contr, coeff_arr = symbolic_2_explicit_worker(angles, j_idx_arr_fs,
        trigonometric_history_arr_fs, angle_history_arr_fs)
    return j_idx_arr_contr, coeff_arr
end

function symbolic_2_explicit_worker(angles, j_idx_arr, trigonometric_history_arr,
        angle_history_arr)
    M = length(angles) # number of roundtrips
...
function explicit_ket_evolution(j_init, angles)
    ...
    j_idx_arr_contr, coeff_arr = sp_2_two_photon(n_bins, j_idx_arr_l, j_idx_arr_m,
        coeff_arr_l, coeff_arr_m)
...
function explicit_final_state_projection_symbolic_backend(N, M, j_out, angles)
    ...
    j_idx_arr_contr, coeff_arr = sp_2_two_photon(N, j_idx_arr_l, j_idx_arr_m,
        coeff_arr_l, coeff_arr_m)

    return j_idx_arr_contr, coeff_arr
end

function explicit_final_state_projection_mesh(N, M, j_out, angles)
    coeff_arr = Vector{ComplexF64}(undef, 0)
    j_idx_arr_contr = Int64[]
    l_out, c_out, m_out, k_out = j2lcmk(N+M, j_out)
    if c_out == 0
