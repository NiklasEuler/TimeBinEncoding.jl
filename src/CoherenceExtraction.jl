...
"""
function coherence_extraction(N, j_out, ρ, angles, noisy_angles=copy(angles),
        contr_j_idxs = correlated_short_bins_idxs(N); extract_diagonal::Bool=true)
	...
	pop_j_out_extracted = explicit_final_state_projection_expval(ρ, j_out, noisy_angles)
...
function recursive_beam_splitter_array!(N_bs, n_idx, m_idx, angles, branch)
    ...
    elseif N_bs == 2
        ...
        angles[m_idx+N_bs_half][n_idx+N_bs+N_bs_quarter:n_idx+N_bs+N_bs_half-1] .= π/2
        # right flank transparent couplers
        recursive_beam_splitter_array!(N_bs_quarter, n_idx+N_bs_quarter,
            m_idx+N_bs_half+N_bs_quarter, angles, "early") # left flank beam splitter array
        recursive_beam_splitter_array!(N_bs_quarter, n_idx+N_bs+N_bs_quarter,
            m_idx+N_bs_half+N_bs_quarter, angles, "late") # right flank beam splitter array
        recursive_beam_splitter_array!(N_bs_half, n_idx+N_bs_half,
            m_idx+N_bs_half, angles, "center")
    end
end
